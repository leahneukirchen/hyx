...
void die(char const *s) __attribute__((noreturn));  /* term.c */
void pdie(char const *s) __attribute__((noreturn)); /* term.c */

static inline size_t min    (size_t x, size_t y) { return x < y ? x : y; }
static inline size_t max    (size_t x, size_t y) { return x > y ? x : y; }
static inline size_t absdiff(size_t x, size_t y) { return x > y ? x - y : y - x; }
